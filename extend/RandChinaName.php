...
class RandChinaName
{
    ...
    public function getName($_number = 1, $_length = 3)
    {
        $result = [];

        for ($i=0; $i < $_number; $i++) {
            $result[$i] = $this->surname[mt_rand(0, count($this->surname) - 1)];

            if ($_length > 1) {
                for ($j=0; $j < $_length - rand(1, $_length - 1); $j++) {
                    $result[$i] .= $this->name[mt_rand(0, count($this->name) - 1)];
                }
            }
        }
