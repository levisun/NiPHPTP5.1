 ...
 * @version   CVS: $Id: UploadFile.php v1.0.1 $
 * @link      www.NiPHP.com
 * @since     2017/10/18
 */
/*
function file_upload($file, $save_path, $validate = array())
{
    ...
    $upload = new UploadFile($file['tmp_name']);
    $result =
...
class UploadFile extends SplFileObject
{
    ...
    public function hash($type = 'sha1')
    {
        if (!isset($this->hash[$type])) {
            ...
            // $this->hash[$type] = $type($this->info['tmp_name'] . $this->filename);
        }
    ...
    protected function buildSaveName($savename)
    {
        if (true === $savename) {
            ...
            if ($this->rule instanceof Closure) {
                ...
                switch ($this->rule) {
                    case 'date':
                        $savename = date('Ymd') . DIRECTORY_SEPARATOR . md5($this->info['tmp_name'] . microtime(true));
                        break;
                    default:
