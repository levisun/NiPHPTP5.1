...
CREATE TABLE IF NOT EXISTS `np_pay` (
  ...
  PRIMARY KEY (`id`),
  KEY `status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT '支付方法表';
...
CREATE TABLE IF NOT EXISTS `np_account_log` (
  ...
  `amount` int(11) NOT NULL DEFAULT '0' COMMENT '流入流出金额',
  `integral` int(11) NOT NULL DEFAULT '0' COMMENT '流入流出积分',
  `status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '状态 1充值 2支付 3提现 4转账',
  `create_time` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
  `update_time` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  KEY `order_id` (`order_id`),
  KEY `status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT '会员帐户流水';
